...
@Controller
public class ProductReviewFileUploadController {
	private static final String command = "/upload_review.prd";
	...
	public void doAction(HttpServletRequest request, MultipartHttpServletRequest multi, HttpServletResponse response) {
		...
		if (upFile.getSize() > 0 && StringUtils.hasLength(upFile.getName())) {
			try {
				...
				String uploadPath = servletContext.getRealPath("/resources/product/image/review/");
				...
				String fileUrl = request.getContextPath()+"/resources/product/image/review/"+sdf.format(date)+"_"+fileName;
